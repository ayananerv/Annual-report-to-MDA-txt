...
from ..config import setup_listener, read_log, setup_root_logger, JobConfig
...
class BatchCorruptRetry(PipelineStage):
    def process(self, stage_context: PipelineContext) -> PipelineContext:
        ...
        if done:
            ...
            log_merge_standard_batch_corrupt(cast(Path, stage_context.result), log_path)
            print("提交下一阶段")
...
def log_merge_standard_batch_corrupt(log_path1: Path, log_path2: Path):
    """
    按照指定逻辑合并两个日志文件：
    1. 读取两个日志文件。
    2. 提取 log1 中错误码为 404 的记录。
    3. 用 log2 中的记录更新上述 404 记录（若 log2 中存在），否则丢弃。
    4. 将 log1 中非 404 的记录与更新后的记录合并。
    """
    ...
    data1 = data1.drop_duplicates(subset=5, keep="first")
    ...
    is_404 = data1[6] == ErrorCode.TIMEOUT.code
    data3 = data1[is_404].copy()
    data1_preserved = data1[~is_404].copy()
    ...
    if not data2.empty:
        ...
        # 筛选 data2：保留那些文件名在 data3 (404列表) 中的行 -> data4
